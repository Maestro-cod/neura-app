...
import { colors, fonts, spacing, zoneColors } from "@/src/theme";
...
export default function Galaxy() {
  ...
  // Enrich zones with fallback colours from theme so GalaxyCanvas always
  // has a colour value even if the DB row has color = null.
  const enrichedZones = useMemo<Zone[]>(
    () =>
      zones.map((z) => ({
        ...z,
        color: z.color || zoneColors[z.name] || "#888888",
      })),
    [zones]
  );
  ...
  return (
    <View style={styles.root}>
      {/* 3D Neuron Galaxy – receives live zone data so planets use real colors */}
        ...
        <GlassCard strong style={styles.insightCard}>
          ...
          {loadingInsight ? (
            ...
            <Text style={styles.insightText} testID="ai-insight-text">
              {insight || "All clear in your galaxy."}
            </Text>
          )}
        </GlassCard>
      </View>
